val input: List<String> = listOf(
        ...
        "pid:849044092 eyr:2020 hgt:186cm iyr:2014 byr:1991 hcl:#866857",
...
val validFields = listOf(
        PassportField("byr", false) { it?.toInt() in 1920..2002 },
        PassportField("iyr", false) { it?.toInt() in 2010..2020 },
        PassportField("eyr", false) { it?.toInt() in 2020..2030 },
        PassportField("hgt", false) {
            when {
                it?.contains("cm", true) == true -> {
                    it.substringBefore("cm").toInt() in 150..193
                }
                it?.contains("in", true) == true -> {
                    it.substringBefore("in").toInt() in 59..76
                }
                else -> {
                    false
                }
        ...
        PassportField("hcl", false) { it?.matches(Regex("#((\\d|[a-f]){6})")) ?: false },
        PassportField("ecl", false) { it in listOf("amb", "blu", "brn", "gry", "grn", "hzl", "oth") },
        PassportField("pid", false) { it?.matches(Regex("[0-9]{9}")) ?: false },
        PassportField("cid", true) { true }
)
