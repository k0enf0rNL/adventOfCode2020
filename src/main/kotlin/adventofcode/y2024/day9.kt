...
data class ProcessStats(
    ...
    val movedFilesIds: MutableList<Int> = mutableListOf()
)
...
fun main() {
    ...
    val processResultPart2 = input.foldIndexed(ProcessStats(startId = 0, endId = (input.size / 2), endIndex = input.lastIndex)) { index: Int, acc: ProcessStats, blockSize: Int ->
        acc.apply {
            if (index % 2 == 0) {
