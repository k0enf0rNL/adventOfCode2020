...
fun NumberOnRow.isInRange(specialCharacter: SpecialCharacter): Boolean =
    listOf(specialCharacter.rowNumber, specialCharacter.rowNumber + 1, specialCharacter.rowNumber - 1).contains(rowNumber) && specialCharacter.index in (indices.first -1..indices.last +1)
...
val input: List<String> = this::class.java.classLoader.getResource("2023/day3.txt")!!.readText().split(System.lineSeparator()).filter { it.isNotBlank() }
