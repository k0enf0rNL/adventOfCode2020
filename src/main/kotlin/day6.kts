...
fun <A> Iterable<A>.split(predicate: (A) -> Boolean) = fold(mutableListOf(mutableListOf<A>())) { acc, e ->
    acc.last() += e
    if (predicate(e)) acc += mutableListOf<A>()
    acc
}

input.split { it == "" }.map {
    it.filter { it != "" }.map { Answers(it) }.let {
        Group(it)
    }
}.also {
    it.sumOf { it.uniqueAnswers() }.also {
        println("Part 1: $it")
    }
}.also {
            it.sumOf { it.allAnswers() }.also {
                println("Part 2: $it")
            }
        }

//Dit was makkelijker geweest met intersect en union icm reduce
